public class Customer {
    ...
    public boolean feelingConfident(){
        ...
        if(beersThatTheyDrunk > 5 && hasCondom && isDTF()){
            return true;
        } else return false;
    }
    ...
    public double tipTheCuteStaff(Staff staff){
        ...
        if (getBeersThatTheyDrunk() * tippingAmount <= getEuros()){
            setEuros(getEuros() - (getBeersThatTheyDrunk() * tippingAmount));
            System.out.println("you have tipped" + getBeersThatTheyDrunk() * tippingAmount);
        }else {
            setEuros(0);
        }
        return getEuros();
